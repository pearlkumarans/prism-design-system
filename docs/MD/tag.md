...
> **No close button (`show-close="false"`):** the 4px close-side pad assumes a 16×16 close button, so without one the trailing edge reads too tight. In that case the close-side pad mirrors the size's base text-side pad — 4 / 6 / 8 (Small / Medium / Large) — across every leading mode (None, Status Indicator, and Icon). Small stays 4/4.
