...
/**
 * Script detail — a DEX remediation-script drill-down (L04), a Patterns::ListDetail
 * instance: record header (script · language/status) over an Overview bento (script
 * code, targets, recent runs). Data from PrismAPI.dex.script().
 */
const RUN_STATE = { Completed: 'success', Failed: 'critical', Running: 'alert' };

export default class ScriptDetailView extends Component {
  @service api;
  ...
  async reload() {
    ...
    try {
      ...
      this.data = dex ? await dex.script({}) : null;
      if (!this.data) throw new Error('No script data');
    } catch (e) {
      this.error = e;
    } finally {
  ...
  get header() { return { icon: 'terminal-square', title: this.data?.name || 'Script' }; }
  get breadcrumbs() {
    return [{ label: 'DEX', href: '#' }, { label: 'Scripts', href: '#' }, { label: this.data?.name || 'Script' }];
  }
  get summary() {
    const d = this.data || {};
    return [
