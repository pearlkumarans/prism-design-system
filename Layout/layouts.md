...
**Form fields (hard rule — use the component's own label; left + medium are the defaults; uniform spacing).** Every form field must render its label through the component's **built-in `label` attribute** — never hand-roll a caption (`<span class="…__label">`, a sibling `<label>`, or a grid column of labels) beside a field set to `label-position="none"`. Hand-rolled labels drift out of vertical alignment, skip the component's `required`/`aria` wiring, and duplicate what the component already does. Defaults for form fields: **`label-position="left"`** and **`size="medium"`** — these are the components' native defaults, so *don't set them at all* unless a specific field needs to deviate (and then say why). All Prism form fields share a **280px left-label column** and stack below 520px, so left labels line up across a mixed form automatically. Set `required` on the component for the `*` marker (don't append `*` to the label text). Date/time inputs use **`ds-date-picker`** / **`ds-time-picker`** — never a `ds-text-input` with a `YYYY-MM-DD` / `HH:mm` placeholder. `ds-button-group` has no left label; give it one with `label-position="left"` (it now supports the same 280px column) — its `show-select-all` toggle renders as a `ds-text-link`.

**Field spacing (uniform gap, no reserved slots).** Lay the fields out in a flex column with a **single uniform gap of `20px`** between fields (`.fields { display:flex; flex-direction:column; gap:20px }`) — the gap sits *after* each field's real content. Do **not** reserve an empty helper-row slot per field (no `padding-bottom` + `:has(ds-field-helper)` trick). An empty `ds-field-helper` collapses to zero on its own, so a field with a note (helper text) keeps the note inside its box and still gets the same 20px gap to the next field as a field without one — a note never crowds the following field, and every field is spaced identically. This is the uniform style for **every** form page and modal form (L06/L07 and any generated form).
