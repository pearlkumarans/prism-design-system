...
> **Label guidance (important).** State is conveyed by the switch itself (thumb position + colour), not by words. Name the *setting* with a **separate field label** beside the switch — a concise noun phrase ("Drive encryption", "Wi-Fi") that stays the same on and off. The optional in-track `Show Text` label is a **short, static** word (like the default "Enable") — it does **not** flip with state. **Never** put a verb phrase, instruction, or sentence in it ("Encrypt on deploy", "Back up to Active Directory") — that reads like a button and, because the text is static, misleads when the switch is off. If you need to explain what on/off does, use a helper line under the field, not the in-track label. (Matches Apple HIG / Material / Carbon / Polaris switch conventions.) The component emits a `console.warn` when the in-track `text` looks like a phrase/sentence (more than two words, or over ~16 characters) to catch this in development.
...
```
<ds-toggle
  label="Enable notifications"     (visible FIELD label beside the switch — names the setting; also the accessible name)
  label-position="end | start"    (default: end — label after the switch; start = before it)
  size="small | medium | large"   (default: medium)
  checked                          (boolean — default OFF)
  disabled                         (boolean)
  show-text                        (boolean — show the short in-track word)
  text="Enable"                    (in-track static word, Show-Text mode; default "Enable")
  rtl                              (boolean — also auto from [dir="rtl"])
></ds-toggle>
<!-- event: ds-toggle-change → detail { checked } -->
```

The **`label`** attribute renders the setting's name as a field label to the side of the
switch (clicking it toggles the switch, and it becomes the control's accessible name via
`aria-labelledby`) — this is the "separate field label beside the switch" the guidance above
recommends. It is distinct from the optional in-track `text` (a short static word inside the
track). The switch is **off by default** (`checked` absent).

### Reflected properties (JS)

Each mirrors its attribute (get/set), so it can be driven imperatively:

| Property | Type | Notes |
|---|---|---|
| `el.checked` | boolean | On/off state (default `false`). |
| `el.label` | string | Field-label text; `''`/`null` removes it. |
| `el.disabled` | boolean | Non-interactive when `true`. |

```js
const t = document.querySelector('ds-toggle');
t.label = 'Enable notifications';   // renders the field label
t.checked = false;                  // default off
t.disabled = true;
t.addEventListener('ds-toggle-change', (e) => console.log(e.detail.checked));
```

For no visible label (e.g. a settings row where the name lives in a separate element),
set `aria-label` on the host instead — it becomes the switch's accessible name.
